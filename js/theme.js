                ...
                $.ajax({
                    ...
                    success: function (json) {
                        if(typeof json['alert'] !='undefined' && json['alert'] != '') {
                            ...
                            if(pathname == 'preview') {
                                if (window.opener) {
                                    // window.location.href = '/npf-backend/module/documents/manage-landing-page';
                                    // opener.location.href = '/npf-backend/module/documents/manage-landing-page';
                                    try {
                                            window.opener.top.location.href = "/npf-backend/module/documents/manage-landing-page";
                                    } catch (e) {
...
$(document).on('click','#CaptchaRefreshBtn',function(){
    var d = new Date();
    var n = d.getTime(); 
    var setCaptchaSrc = function () {
        $("#CaptchaImage").attr('src', jsVars.CaptchaLink + '?' + n);
    };

    if (typeof jsVars !== 'undefined' && jsVars.CaptchaCheckLink) {
        ...
        var successCallback = function(resp){
            ...
            if (resp && resp.status === 'Fail') {
                var msg = (typeof resp.error === 'string' && resp.error.length) ? resp.error : 'Too many attempts. Please try again later.';
                if ($("#captchaError").length) {
                    $("#captchaError").text(msg);
                } else if (typeof alertErrorPopup === 'function') {
                    alertErrorPopup(msg);
                } else if (typeof alertPopup !== 'undefined') {
        ...
        send_ajax_request(jsVars.CaptchaCheckLink, 'get', successCallback,completeCallBack,beforeSendCallback,errorCallBack);
    } else {
        setCaptchaSrc();
    }
});
